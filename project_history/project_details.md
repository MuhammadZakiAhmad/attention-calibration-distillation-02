...
[S2-2026-09-17-1228] Negation-as-failure records fail BOTH conditions, making them permanently non-Golden — where: Colab smoke test, `AttNeg-CWA-D2-104::Q5` (gold True; zs False, fs False → both_failed) and `AttNeg-CWA-D2-1067::Q4` (gold False; zs False, fs True → regression) — status: resolved — tried: the gold proof for Q5 is `[(((((triple16 triple15) -> rule1) NAF) -> rule2))]`, deriving `Erin is not nice` from the *absence* of `Erin is nice`. Confirmed against the raw dataset row that `triples` contains **no negative triple at all** and `theory` (taken verbatim at `phase1.py:485`) contains no negative fact — the `nafN` entry is a *derived* intermediate under the closed-world assumption, not a premise the renderer dropped. So `phase1.py` is not at fault; the `p1-v1` system message simply never licensed a negation-by-absence inference, and both models behaved reasonably given it — zero-shot answered "the context does not provide information about Erin's niceness", the Teacher confabulated the opposite ("we know that Erin must be nice"). Exact count: **81 / 500 (16.2%)** of the pool has an explicit `NAF` step (47 at QDep 2, 34 at QDep 1). Fixed by stating the closed-world assumption in the system message (`p1-v2`). A first estimate of 57% was wrong — it came from a regex that flagged any negatively-phrased premise absent from context, which over-counts because a question like "Gary is not round" pairs with a rule mentioning "not rough" without needing NAF at all; the exact test is whether the proof string contains `NAF`.

[S2-2026-09-17-1228] Smoke test drawn from an unrepresentative head of the pool — where: Colab `--limit 2` smoke test — status: resolved (no code change needed) — tried: the first 2 pool records are **100% NAF**, against 16.2% pool-wide, so the smoke produced `golden 0 / both_failed 1 / regression 1` and looked like a total technique failure when it was a 2.6%-probability draw. Measured: first 20 records are 30% NAF, first 50 are 28%. Fixed by using `--limit 20` for future smoke tests. The lesson generalises: on a pool with any structure, a `--limit 2` smoke tests the pipeline's mechanics but tells you nothing about the distribution — read it as a plumbing check, never as a signal about outcomes.
...
[S2-2026-09-17-1228] `model.dtype` changed from `bfloat16` to `float16` for Phase 1 — why: the Phase 1 run executes on a Colab Tesla T4 (compute capability 7.5, Turing, verified with `nvidia-smi`). bf16 has no hardware support below compute capability 8.0, so every bf16 op on a T4 takes a slow emulated fallback path — numerically correct but several times the wall clock, which is decisive when a Colab session will not outlive a multi-hour run. T4 *does* have native fp16 tensor cores, and Phase 1 is inference-only, so there is no optimizer or GradScaler to complicate it. Recorded as an explicit configuration change per §24. Not in §24's forbidden list (rank, target layers, loss, tokenizer, prompt format, dataset condition, answer mapping) — rejected: keeping bf16 and accepting the slowdown (would push a 2–5h run past the session limit and make resumption the common case rather than the exception), acquiring an L4/A100 runtime (would remove the compromise entirely, but is not available on this Colab tier) — reversible: **no, not after generation begins** — the dtype is part of what makes the cached generation reproducible. Phase 2 should target compute capability >= 8.0 rather than inherit this compromise, since training would also need a GradScaler in fp16.

[S2-2026-09-17-1228] System message bumped to `p1-v2`, adding an explicit statement of the closed-world assumption — why: 81 / 500 pool records (16.2%) have a gold proof containing an explicit `NAF` step, and for those the negative premise appears nowhere in the context — it must be *derived* from the absence of the positive fact. `p1-v1` never licensed that inference, so both conditions failed those records for reasons that have nothing to do with the technique under test, and converting a failed record into a Golden Record was impossible in principle rather than merely unlikely. The clause is a property of the dataset configuration (`CWA-depth-2`), true of every record and identical for Teacher and Student, so it conveys no target-specific information and satisfies §44 Rule 2 — the same category of statement as "answers are True or False" — rejected: **materialising the `nafN` intermediates into the context** (they come from `proofsWithIntermediates`, the same source as the gold proofs, so injecting them would hand the model reasoning steps it is supposed to produce — a direct §7/§44 Rule 2 leak, and it would also change the dataset condition), **filtering the pool down to non-NAF records** (loses 16% of a pool §51 fixes at exactly 500, and swaps a prompt problem for a data problem — the NAF records are legitimate instances of the task) — reversible: **yes, and cheaply *right now*** — no generation predates this on a clean run, but once a full p1-v1 generation exists, reverting means discarding it. **Judgement call, recorded as such**: this makes the task *well-posed* rather than *easier* — the model still has to notice which properties are absent and chain the inference; only the licence to do so is now stated. It must be validated by smoke test before committing to a full run.
